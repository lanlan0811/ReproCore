...
2. 按 `docs/releasing.md` 配置 `NPM_TOKEN`、Gitee 发布 Token、目标变量和仓库保护；
   凭据缺失会在任何检出、构建或发布前失败。
3. 确认 GitHub CI 在 Windows、macOS、Linux 和 Node 22.13/24 全绿。
4. 所有门禁满足后更新 CHANGELOG 日期，再创建并向两个远端推送 `v0.1.0` tag。
