...
独立试用统一使用最新成功 `master` CI 上传的、名称中包含完整提交 SHA 的候选
artifact。正式打标签前，维护者必须确认至少两份有效反馈引用同一个待发布提交。

不得通过手工上传替代工作流，也不得重用失败运行产生的局部产物。若任一渠道失败，版本
仍视为未完成发布，修复原因后重新运行同一 tag 的完整工作流。重跑会下载并校验 Gitee
同名附件的 SHA-256；内容一致才跳过，内容不同则删除旧附件并上传本次构建产物。
